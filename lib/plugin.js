...
ManifestPlugin.prototype.apply = function(compiler){ // eslint-disable-line max-lines-per-function
    ...
    const emit = (compilation, compileCallback) => { // eslint-disable-line max-lines-per-function
        ...
        files.forEach(file => {
            file.isInHead = file.isInBody = false;
            const filePath = `${compilation.options.output.publicPath}${file.path}`;
            for (const {head, body} of htmlPluginDataArr) {
                for (const element of head) {
                    const url = element.attributes.href || element.attributes.src;
                    if (filePath === url) {
                        file.isInHead = true;
                        Object.assign(file, element);
                        break;
                    }
                }
